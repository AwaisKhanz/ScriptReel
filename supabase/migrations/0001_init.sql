-- 0001_init.sql — ScriptReel core schema (doc 05).
-- v1 is single-user, local-first: RLS is DISABLED. projects.user_id exists for
-- forward-compatibility but is unused. pg-boss owns its own `pgboss` schema and
-- must never be touched here. Media/renders live on local disk (DATA_DIR), not in
-- the DB; only relational state lives here.
...
create table projects (
  ...
  cancel_requested boolean not null default false,      -- worker polls this between stages (doc 06)
  error            jsonb,                               -- {stage, code, message} on failure
  created_at       timestamptz not null default now(),
  updated_at       timestamptz not null default now()
);

create table pipeline_runs (                            -- one row per stage per project
  id          uuid primary key default gen_random_uuid(),
...
create table candidates (
  ...
  width       int,
  height      int,
  duration    numeric,                                  -- null for images
...
-- Indexes -------------------------------------------------------------------
create index beats_project_idx    on beats (project_id, idx);
create index candidates_beat_rank on candidates (beat_id, rank);
create index pipeline_runs_project on pipeline_runs (project_id);
create index asset_cache_last_used on asset_cache (last_used_at);   -- eviction (doc 14)

-- Realtime ------------------------------------------------------------------
-- UI subscribes to row changes filtered by project_id (doc 05 §Realtime).
alter publication supabase_realtime add table projects, pipeline_runs;
