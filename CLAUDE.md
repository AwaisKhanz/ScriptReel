...
Prereqs and model downloads: `docs/19-SETUP-MACOS.md`. **The DB is a local portable Postgres** (`C:\Users\GNG\scriptreel-pg`, port 54322, no Docker/service — moved off Supabase Cloud 2026-07-18 to escape the session-pooler's 15-connection EMAXCONNSESSION). `pnpm dev` auto-starts it (`scripts/ensure-local-db.mjs`, keyed on `LOCAL_PG_DIR` in `.env`); `stop.cmd` in that folder stops it. `DATABASE_URL` selects backend and `packages/db/client.ts` auto-detects localhost (no SSL, pool 10) vs Cloud (SSL, pool 3) — the Cloud line is kept commented in `.env` for one-line rollback. New migrations apply with `psql -f supabase/migrations/NNNN.sql` against :54322 (NOT `supabase db push`, which targets the still-linked Cloud project). Local was bootstrapped with stub `anon`/`authenticated`/`service_role` roles + a `supabase_realtime` publication so all 13 migrations apply unmodified.

## Architecture in one paragraph

Three local processes: **web** (Next.js 15 — UI + thin API routes), **worker** (Node 22 — the pipeline, providers, FFmpeg), **sidecar** (Python 3.12 FastAPI — models only: Kokoro TTS, SigLIP 2 embeddings, mlx-whisper alignment, Pillow text cards, optional FLUX). A fourth optional process, the **voice server** (`services/voice`, port 8585), serves the natural cloned narrators (Chatterbox) over the same `/tts` contract; started by `pnpm dev`, model loaded lazily on first use. Postgres runs **locally** (portable Postgres on :54322, no Docker — see Commands; reversible to Supabase Cloud via `DATABASE_URL`); pg-boss for jobs; media and renders on disk under `DATA_DIR`. Stages: `analyze → [search → score ∥ tts] → fetch → [review gate] → align → compose` (search→score and tts run concurrently; fetch runs **before** the review gate so the storyboard previews the real stitched clip).
...
- TTS is engine-swappable **per voice**, not by env: each `Voice` in `packages/core/src/voices.ts` declares `engine: 'kokoro' | 'chatterbox'`. Kokoro → sidecar (`SIDECAR_URL`); Chatterbox → the isolated voice server (`services/voice`, `CHATTERBOX_URL`, port 8585). Both speak the **same** `/tts` (`{text,voice,langCode,speed,outPath}` → `{path,durationSec}`, 24 kHz mono PCM_16), so the tts stage, drift assert, and alignment are engine-agnostic. The 4 named narrators (usama/awais/noman/adeel) are the UI's only voices (the 54 Kokoro voices stay in the registry for existing projects); their reference clips are public-domain LibriVox readers at `services/voice/voices/<id>.wav` → Chatterbox is MIT → output is commercial-safe. Add a voice: drop a ~15 s 24 kHz mono clip + one row in `voices.ts` (no migration — voice is a jsonb setting; changing it invalidates `tts → align → compose`). `services/voice` pins a different cu128 torch than the sidecar, so it MUST stay a separate process. Chatterbox pacing is baked (exaggeration 0.4 / cfg_weight 0.3, calm/mature); the `speed` slider is a Kokoro-only knob.
